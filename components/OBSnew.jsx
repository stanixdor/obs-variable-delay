...
export default function ObsControllerDelay() {
  ...
  const connect = async () => {
    ...
    try {
      ...
      setDelayScene((v) => v || sceneNames.find((n) => n !== "Escena Delay") || sceneNames[0] || "");
  ...
  const syncDelayScene = async () => {
    ...
    try {
      ...
      for (const sceneName of scenes) {
        ...
        if (sceneName === delayScene) continue; // no nos añadimos a nosotros mismos
        // Evitar duplicados
  ...
  const enableDelay = async () => {
    ...
    try {
      ...
      await obs().call("StartRecord").catch(() => {});


      // Paso 1: activamos puente y desactivamos resto
      await disableAllExcept(delayScene, [bridgeInput]);
      ...
      // Paso 2: tras X segundos, cambiar a video delay
      setTimeout(async () => {
        try {
          await setSceneItemEnabledInScene(delayScene, bridgeInput, false);
          await setSceneItemEnabledInScene(delayScene, delayInput, true);
        } catch (e) {
          console.error(e);
        }
      }, delaySec * 1000);
  ...
  const disableDelay = async () => {
    ...
    try {
      ...
      // Ocultamos/muteamos video delay
      await setSceneItemEnabledInScene(delayScene, delayInput, false);
